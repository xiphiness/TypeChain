...
export function codegenAbstractFunctions(options: GenerateFunctionOptions, fns: FunctionDeclaration[]): string {
  if (fns.length === 1) {
    return `${generateFunction(options, fns[0])}${codegenForOverloadedFunctions(options, fns)}`
  }

  return codegenAbstractForOverloadedFunctions(options, fns)
}

function generateAbstractFunction(options: GenerateFunctionOptions, fn: FunctionDeclaration, overloadedName?: string): string {
  return `
  ${generateFunctionDocumentation(fn.documentation)}
  abstract ${overloadedName ?? fn.name}(${generateInputTypes(fn.inputs)}${
...
export function codegenAbstractForOverloadedFunctions(options: GenerateFunctionOptions, fns: FunctionDeclaration[]): string {
  return fns.map((fn) => generateAbstractFunction(options, fn, `"${getSignatureForFn(fn)}"`)).join('\n')
}
