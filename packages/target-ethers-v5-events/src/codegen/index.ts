...
import { AbiParameter, BytecodeWithLinkReferences, Contract, EventArgDeclaration, FunctionDeclaration } from 'typechain'
...
import {
  ...
  generateEvents,
  codegenForEventsTypeMap,
} from './events'


export function codegenContractTypings(contract: Contract) {
  const contractImports: string[] = ['Contract', 'ContractTransaction', 'EventFilter', 'PopulatedTransaction']
  ...
  export type ${contract.name}EventMap = {
    ${codegenForEventsTypeMap(contract.events)}
  }

  export type ${contract.name}Events = ValueOf<${contract.name}EventMap>
...
//   import { ${contract.name} } from "../types/${contract.name}";
